	...
	foreach($display_list as $car) {
		...
		if ($car != null) {
			if (count($search_query) > 0) {
				foreach($search_query as $query) {
					if (stripos($car->Make, $query) !== false ||
						stripos($car->Model, $query) !== false||
						stripos($car->StockNumber, $query) !== false ||
						stripos($car->VIN, $query) !== false ||
						stripos($car->Trim, $query) !== false ||
						stripos($car->BodyStyle, $query) !== false ||
						stripos($car->FuelType, $query) !== false)
