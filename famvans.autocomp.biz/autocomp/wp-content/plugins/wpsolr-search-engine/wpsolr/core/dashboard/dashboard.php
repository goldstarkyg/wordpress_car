	...
	if ( 'solr_settings' === $current_screen->parent_file ) {
		...
		if ( defined( 'WPSOLR_PLUGIN_PRO_DIR' ) ) {
			...
			if ( is_array( $licenses ) && ! empty( $licenses ) ) {
				...
				$footer_version = sprintf( '<div class="wpsolr_activated_packs">Activated packs: %s.</div>', implode( ', ', $licenses ) );
			}
		}
		...
		$footer_text = sprintf( '<div style="float:left;margin:5px;">%s <span>%s</span></div>', $footer_version, $footer_text );
