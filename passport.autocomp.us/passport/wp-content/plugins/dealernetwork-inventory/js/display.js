function Pager(parentName,itemsPerPage){
    ...
    this.showRecords=function(from,to){
        var items=document.getElementById(this.parentName).children;
    ...
    this.showPage=function(pageNumber){
        ...
        var pagerRecCount=document.getElementById("PRC");
        ...
        to=Math.min(to,this.itemsCount-1);
        this.showRecords(from,to);
        pagerRecCount.innerHTML=(from+1)+" - "+(to+1)+" of "+this.itemsCount;
        window.scrollTo(0,0)};
        this.prev=function(){
    ...
    this.init=function(){
        ...
        this.showPageNav=function(pagerName,positionId){
            ...
            var pagerHtml='<span class="left" id="PRC"></span>';
...
function PagerMobile(parentName,itemsPerPage){
    this.parentName=parentName;
    this.itemsPerPage=itemsPerPage;
    this.currentPage=1;
    this.pages=0;
    ...
    this.showPage=function(pageNumber){
        ...
        var to=from+itemsPerPage-1;
        to=Math.min(to,this.itemsCount-1);
        var allpage = Math.ceil(this.itemsCount/itemsPerPage);
        this.showRecords(from,to);
        // pagerRecCount.innerHTML=(from+1)+" - "+(to+1)+" of "+this.itemsCount;
        pagerRecCount.innerHTML= pageNumber+" of "+allpage;
        window.scrollTo(0,0)
    };
    this.prev=function(){
        if(this.currentPage>1){
...
jQuery(document).ready(function(){
    ...
    if(jQuery("#thumbs").length){
     ...
     if(jQuery("input[type=text].year").length){
      ...
      /*gold code*/
        jQuery('.dn-inv-button').bind("mouseover", function(){
            var color  = jQuery(this).css("background-color");
            jQuery(this).css("background", "#edc966");
